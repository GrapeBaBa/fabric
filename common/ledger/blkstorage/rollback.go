...
func Rollback(blockStorageDir string, isMmapEnabled bool, ledgerID string, targetBlockNum uint64, indexConfig *IndexConfig) error {
	r, err := newRollbackMgr(blockStorageDir, isMmapEnabled, ledgerID, indexConfig, targetBlockNum)
	if err != nil {
		return err
	}
	defer r.dbProvider.Close()

	if err := recordHeightIfGreaterThanPreviousRecording(r.ledgerDir, isMmapEnabled); err != nil {
		return err
	}
...
func newRollbackMgr(blockStorageDir string, isMmapEnabled bool, ledgerID string, indexConfig *IndexConfig, targetBlockNum uint64) (*rollbackMgr, error) {
	r := &rollbackMgr{}
...
func (r *rollbackMgr) deleteIndexEntriesRange(startBlkNum, endBlkNum uint64) error {
	...
	stream, err := newBlockStream(r.ledgerDir, r.isMmapEnabled, lp.fileSuffixNum, int64(lp.offset), -1)
	if err != nil {
		return err
	}
	defer stream.close()
...
func (r *rollbackMgr) rollbackBlockFiles() error {
	...
	targetFileNum, err := binarySearchFileNumForBlock(r.ledgerDir, r.isMmapEnabled, r.targetBlockNum)
	if err != nil {
		return err
	}
	...
	endOffset, err := calculateEndOffSet(r.ledgerDir, r.isMmapEnabled, targetFileNum, r.targetBlockNum)
	if err != nil {
		return err
	}
...
func calculateEndOffSet(ledgerDir string, isMmapEnabled bool, targetBlkFileNum int, blockNum uint64) (int64, error) {
	stream, err := newBlockfileStream(ledgerDir, isMmapEnabled, targetBlkFileNum, 0)
	if err != nil {
		return 0, err
	}
	defer stream.close()
...
func ValidateRollbackParams(blockStorageDir string, isMmapEnabled bool, ledgerID string, targetBlockNum uint64) error {
	...
	if err := validateTargetBlkNum(ledgerDir, isMmapEnabled, targetBlockNum); err != nil {
		return err
	}
	return nil
...
func validateTargetBlkNum(ledgerDir string, isMmapEnabled bool, targetBlockNum uint64) error {
