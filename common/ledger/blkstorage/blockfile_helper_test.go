...
func TestConstructBlockfilesInfo(t *testing.T) {
	...
	blkfilesInfo, err := constructBlockfilesInfo(blkStoreDir, conf.isMmapEnabled)
	...
	checkBlockfilesInfoFromFS(t, blkStoreDir, conf.isMmapEnabled, blockfileMgr.blockfilesInfo)
...
func TestBinarySearchBlockFileNum(t *testing.T) {
	...
	env := newTestEnv(t, NewConf(blockStoreRootDir, maxFileSie, true))
	defer env.Cleanup()
	...
	for i := uint64(0); i < 100; i++ {
		fileNum, err := binarySearchFileNumForBlock(ledgerDir, blkfileMgr.conf.isMmapEnabled, i)
		require.NoError(t, err)
...
func checkBlockfilesInfoFromFS(t *testing.T, blkStoreDir string, isMmapEnabled bool, expected *blockfilesInfo) {
	blkfilesInfo, err := constructBlockfilesInfo(blkStoreDir, isMmapEnabled)
	require.NoError(t, err)
